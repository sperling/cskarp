...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests
{
    public class XmlDocCommentTests
    {
        ...
        public void TestEmptyElementNoAttributes()
        {
            ...
            var node = leading[0];
            Assert.Equal(SyntaxKind.SingleLineDocumentationCommentTrivia, node.CSharpKind());
        ...
        public void TestFourOrMoreSlashesIsNotXmlComment()
        {
            ...
            var leading = tree.GetCompilationUnitRoot().EndOfFileToken.LeadingTrivia;
            Assert.Equal(1, leading.Count);
            Assert.Equal(SyntaxKind.SingleLineCommentTrivia, leading[0].CSharpKind());
...
";
            ...
            Assert.Equal(SyntaxKind.SingleLineDocumentationCommentTrivia, leading[0].CSharpKind());
        ...
        public void TestThreeOrMoreAsterisksIsNotXmlComment()
        {
            ...
            var leading = tree.GetCompilationUnitRoot().EndOfFileToken.LeadingTrivia;
            Assert.Equal(1, leading.Count);
            Assert.Equal(SyntaxKind.MultiLineCommentTrivia, leading[0].CSharpKind());
            Assert.Equal(text, leading[0].ToFullString());
        }
...
class C { }";
        ...
        public void TestEmptyElementNoAttributesDelimited()
        {
            ...
            Assert.Equal(1, leading.Count);
            var node = leading[0];
            Assert.Equal(SyntaxKind.MultiLineDocumentationCommentTrivia, node.CSharpKind());
            Assert.Equal(text, node.ToFullString());
            var doc = (DocumentationCommentTriviaSyntax)node.GetStructure();
            Assert.Equal(3, doc.Content.Count);
            Assert.Equal(SyntaxKind.XmlText, doc.Content[0].Kind);
...
class C { }";
            ...
            Assert.Equal(SyntaxKind.ClassDeclaration, tree.GetCompilationUnitRoot().ChildNodesAndTokens()[0].CSharpKind());
            ...
            Assert.Equal(SyntaxKind.EndOfFileToken, tree.GetCompilationUnitRoot().ChildNodesAndTokens()[1].CSharpKind());
        }
...
*/";
            ...
            Assert.Equal(SyntaxKind.XmlTextLiteralNewLineToken, xmltext.ChildNodesAndTokens()[0].CSharpKind());
            Assert.True(xmltext.ChildNodesAndTokens()[0].HasLeadingTrivia);
            Assert.Equal("\r\n", xmltext.ChildNodesAndTokens()[0].ToString());
            Assert.Equal(SyntaxKind.XmlTextLiteralToken, xmltext.ChildNodesAndTokens()[1].CSharpKind());
            Assert.False(xmltext.ChildNodesAndTokens()[1].HasLeadingTrivia);
            Assert.Equal("x", xmltext.ChildNodesAndTokens()[1].ToString());
            Assert.Equal(SyntaxKind.XmlTextLiteralNewLineToken, xmltext.ChildNodesAndTokens()[2].CSharpKind());
